...
class Network(object):
    ...
    def deconv(self,
        ...
        filter = tf.Variable(tf.truncated_normal([kernal_size, kernal_size,output_channel, input_channel], stddev=0.01), name='deconv_filter')
        output_shape = [1, output_h, output_w, output_channel]
        strides = [1, stride_size, stride_size, 1]
        deconv = tf.nn.conv2d_transpose(input, filter, output_shape= output_shape, strides= strides, padding= padding, name=name)
        return deconv
    ...
    @layer
    def bilinear(self, input, size_h, size_w, name):
        # input_w = input.get_shape()[1]
        with tf.variable_scope(name) as scope:
            return tf.image.resize_bilinear(input, size=[size_h, size_w])
