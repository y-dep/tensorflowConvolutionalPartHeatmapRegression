...
SNAPSHOT_DIR = './checkpoint/pheat'
RESTORE_FROM = './model/pheat'
LOG_DIR = './logs/pheat'
...
def main():
    ...
    for i in xrange (num_gpus):
        with tf.device('/gpu:%d' % i):
            with tf.name_scope('Tower_%d' % (i)) as scope:
                ...
                # with tf.variable_scope('', reuse=reuse1):
                #     regression_net = RegressionSubnet({'data': regression_input}, is_training=False, n_classes=N_CLASSES)
                #     regressionnet_out = regression_net.layers['C8_deconv']

                # resized_upheatmap = tf.image.resize_nearest_neighbor(next_heatmap, [380, 380])
                # loss_regression = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(resized_upheatmap, regressionnet_out)), [1, 2, 3])))

                reduced_loss = loss_detection
    ...
    loss_detection = tf.reduce_mean(tf.get_collection('loss_detection'))
    #loss_ave = tf.reduce_mean(tf.get_collection('reduced_loss'))

    loss_summary_detection = tf.summary.scalar("loss_detection", loss_detection)
    # loss_summary_ave = tf.summary.scalar("loss_ave", loss_ave)
    # loss_summary = tf.summary.merge([loss_summary_detection])
